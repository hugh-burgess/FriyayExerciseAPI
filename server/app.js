...
app.get("/courses/:courseId/students/:studentId", (req, res) =>{
  ...
  Student.findById(courseId && studentId).then((students) => {
    ...
    res.status(200);
      ...
      res.json({ error: "Pleases specify the correct course and student IDs" 
      });
     }
      }).catch((error) => {
        res.status(500)
res.json({error: `Internal Server Error ${error}, please enter the correct ID details...`});
      });
...
app.post("/courses", (req, res) => {
  ...
  if (!req.body.name && !req.body.type) {
    ...
    Course.create({ name: req.body.name, type: req.body.type, location: req.body.location }) 
